...
class ClassicalVariationalState(VariationalState):
    ...
    def expect_and_grad(
        ...
        if mutable is False:
            Ō_grad, _ = grad_expect_hermitian(
                self._apply_fun,
                self.parameters,
                self.model_state,
                σ,
            ...
            Ō_grad, self.model_state = grad_expect_hermitian(
                self._apply_fun,
                self.parameters,
                self.model_state,
                σ,
    ...
    # if mutable is False:
    #    _, vjp_fun = nkjax.vjp(
    #        lambda w: self._apply_fun(
    #            self.model, {"params": w, **self.model_state}, σ
    #        ),
    #        self.parameters,
    #        conjugate=True,
    #    )
    # else:
    #    _, vjp_fun, self.model_state = nkjax.vjp(
    #        lambda w: self._apply_train_fun(
    #            self.model, {"params": w, **self.model_state}, σ, mutable=mutable
    #        ),
    #        self.parameters,
    #        conjugate=True,
    #        has_aux=True,
    #    )
    # Ō_grad = vjp_fun(O_loc / self.n_samples)[0]
    #
